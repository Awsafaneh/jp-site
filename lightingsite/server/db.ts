...
import { InsertUser, users, categories, products, contactSubmissions, InsertContactSubmission } from "../drizzle/schema";
...
export async function getProducts(categoryId?: number, limit: number = 20, offset: number = 0) {
  const db = await getDb();
  if (!db) return [];
